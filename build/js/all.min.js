...
!function(){function e(){o(),t(),i()}function t(){var e=window.innerWidth/m.soap.width;Stage(function(t){function n(){setTimeout(function(){var e=document.getElementsByTagName("canvas")[0].toDataURL();m.zoom.img=new Image,m.zoom.img.src=e},500)}m.canvas.zoom=document.getElementsByTagName("canvas")[1];var o=m.canvas.zoom;o.width=m.zoom.size*window.devicePixelRatio,o.height=o.width;o.getContext("2d");m.canvas.main=document.getElementsByTagName("canvas")[0];m.canvas.main.getContext("2d");t.viewbox(window.innerWidth,window.innerHeight);var i=Stage.image("soap").appendTo(t);i.pin({alignX:-.5,alignY:-.5,scale:.09*e});var a=500;i.tween(a).ease("bounce").pin({alignX:.5,alignY:.9,scale:.9*e,rotation:2*Math.PI});for(var l=0;l<m.needles.cnt;++l){var d=Math.PI/10*Math.random()-Math.PI/20,s=.6+.1*Math.random(),c=.2+l/10,u=2,h=u*Math.tan(d),p=120;m.needles.holes.push({x:c*window.innerWidth+p*Math.tan(d),y:s*window.innerHeight-p});var g=Stage.image("needle").appendTo(t).pin({alignX:c+h,alignY:s-u,scale:.6*e,rotation:d}),w=200;g.tween().delay(a+w*l).ease("in").pin({alignX:c,alignY:s}).done(n)}r()})}function n(e,t){var n=m.canvas.zoom,o=n.getContext("2d"),i=n.width,a=n.height,r=2*(e-n.width/4),l=2*(t-n.height/4);m.zoom.img&&(o.clearRect(0,0,m.canvas.main.width,m.canvas.main.height),o.drawImage(m.zoom.img,r,l,i,a,0,0,i,a))}function o(){Stage({image:"img/soap-500.png",textures:{soap:{x:0,y:0,width:m.soap.width,height:m.soap.height}}}),Stage({image:"img/needle.png",textures:{needle:{x:0,y:0,width:33,height:500}}})}function i(){m.canvas.particle=document.getElementById("particle");var e=m.canvas.particle;e.width=window.innerWidth,e.height=window.innerHeight,m.particle.proton=new Proton,m.particle.renderer=new Proton.Renderer("canvas",m.particle.proton,e),m.particle.renderer.start(),a()}function a(){++m.particle.colorId;for(var e=0,t=m.particle.emitters.length;t>e;++e){var n=Color.parse("hsl("+(m.particle.colorId+Math.floor(360/m.needles.cnt*e))%360+", 100%, 50%)").hexTriplet(),o=Color.parse("hsl("+(m.particle.colorId+Math.floor(360/m.needles.cnt*(e+1)))%360+", 100%, 50%)").hexTriplet();m.particle.colorBehaviours[e].reset(n,o)}m.particle.proton.update(),requestAnimationFrame(a)}function r(){var e=new Proton.Emitter;m.particle.emitters.push(e),e.rate=new Proton.Rate(Proton.getSpan(10,20),.1),e.addInitialize(new Proton.Radius(1,12)),e.addInitialize(new Proton.Life(.1,.5)),e.addInitialize(new Proton.Velocity(3,Proton.getSpan(90,90),"polar")),e.addInitialize(new Proton.Radius(5));var t=Color.parse("hsl("+360/m.needles.cnt*m.needles.curId+", 100%, 50%)").hexTriplet(),n=Color.parse("hsl("+360/m.needles.cnt*(m.needles.curId+1)+", 100%, 50%)").hexTriplet(),o=new Proton.Color(t,n);if(m.particle.colorBehaviours.push(o),e.addBehaviour(o),e.addBehaviour(new Proton.Alpha(1,0)),e.addBehaviour(new Proton.Scale(1,.5)),m.needles.curId>=0){var i=l(),a=m.needles.holes[m.needles.curId];e.p.x=a.x,e.p.y=a.y;var r=s(i,a);m.particle.emitters[0].addInitialize(new Proton.Life(.008*r,.02*r));var d=90-c(i,a);e.addInitialize(new Proton.Velocity(3,Proton.getSpan(d-20,d+20),"polar")),e.emit()}m.particle.proton.addEmitter(e)}function l(){return m.needles.curId>0?m.needles.holes[m.needles.curId-1]:{x:0,y:3*window.innerHeight/4}}function d(){return m.needles.curId>=0?m.needles.holes[m.needles.curId]:{x:0,y:3*window.innerHeight/4}}function s(e,t){return Math.sqrt((e.x-t.x)*(e.x-t.x)+(e.y-t.y)*(e.y-t.y))}function c(e,t){if(e.x===t.x)return t.y-e.y>=0?0:180;var n=Math.atan(-(t.y-e.y)/(t.x-e.x));return t.x-e.x<0&&(n+=Math.PI),n/Math.PI*180+180}function u(){m.time.end=new Date;var e=m.time.end-m.time.start;document.getElementById("time").innerHTML=e/1e3,document.getElementById("text").style.display="block",m.canvas.zoom.style.display="none",document.getElementById("touch-position").style.display="none"}function h(){window.location.reload(!0)}var m={soap:{width:500,height:290},zoom:{size:300,img:null},mouse:{isDown:!1},needles:{holes:[],curId:-1,cnt:7},particle:{proton:null,emitters:[],renderer:null,colorId:0,colorBehaviours:[]},canvas:{zoom:null,main:null,particle:null},time:{start:null,end:null}};e(),window.restart=h,document.addEventListener("touchstart",function(e){m.mouse.isDown=!0,null===m.time.start&&(m.time.start=new Date,document.getElementById("description").style.display="none"),null!==m.time.end&&(m.canvas.zoom.style.display="block",document.getElementById("touch-position").style.display="block",n(e.touches[0].clientX,e.touches[0].clientY)),m.particle.emitters[0].emit()}),document.addEventListener("touchend",function(){m.mouse.isDown=!1,m.canvas.zoom.style.display="none",document.getElementById("touch-position").style.display="none",m.particle.emitters[0].stopEmit()}),document.addEventListener("touchmove",function(e){if(m.mouse.isDown){var t=e.touches[0].clientX,o=e.touches[0].clientY,i=10,a=m.needles.holes[m.needles.curId+1];a&&Math.abs(a.x-t)<i&&Math.abs(a.y-o)<i&&(++m.needles.curId,r(),m.needles.curId===m.needles.cnt-1&&u()),n(t,o);var l=m.particle.emitters[0];l.p.x=t,l.p.y=o;var h=d(),p={x:t,y:o},g=s(h,p);m.particle.emitters[0].addInitialize(new Proton.Life(.005*g,.01*g));var w=90-c(h,p);l.addInitialize(new Proton.Velocity(3,Proton.getSpan(w-2,w+2),"polar"))}e.preventDefault()})}();
